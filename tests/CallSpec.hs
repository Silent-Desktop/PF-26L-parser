  ...
  describe "call" $ do
    describe "no arguments" $ do
      it "parses empty call" $
        statement "" "f()" `shouldBe` Right (Call "f" [])
    describe "positional arguments" $ do
      it "parses single number arg" $
        parse call "" "f(1)" `shouldBe` Right (Call "f" [PosArg (Number 1)])
      it "parses single variable arg" $
        parse call "" "f(x)" `shouldBe` Right (Call "f" [PosArg (Var "x")])
      it "parses single string arg" $
        parse call "" "f(\"hello\")" `shouldBe` Right (Call "f" [PosArg (StringLit "hello")])
      it "parses multiple positional args" $
        parse call "" "f(x, 1, \"hello\")" `shouldBe` Right (Call "f" [PosArg (Var "x"), PosArg (Number 1), PosArg (StringLit "hello")])
      it "parses math expr as arg" $
        parse call "" "f(x + 1)" `shouldBe` Right (Call "f" [PosArg (Add (Var "x") (Number 1))])
    describe "keyword arguments" $ do
      it "parses single kwarg" $
        parse call "" "f(x=1)" `shouldBe` Right (Call "f" [KwArg "x" (Number 1)])
      it "parses single kwarg with string" $
        parse call "" "f(x=\"hello\")" `shouldBe` Right (Call "f" [KwArg "x" (StringLit "hello")])
      it "parses multiple kwargs" $
        parse call "" "f(x=1, y=2)" `shouldBe` Right (Call "f" [KwArg "x" (Number 1), KwArg "y" (Number 2)])
    describe "mixed arguments" $ do
      it "parses positional then keyword" $
        parse call "" "f(1, x=2)" `shouldBe` Right (Call "f" [PosArg (Number 1), KwArg "x" (Number 2)])
      it "parses multiple positional then keyword" $
        parse call "" "f(1, 2, x=3)" `shouldBe` Right (Call "f" [PosArg (Number 1), PosArg (Number 2), KwArg "x" (Number 3)])
      it "fails with positional after keyword" $
        parse call "" "f(x=1, 2)" `shouldSatisfy` isLeft
    describe "invalid input" $ do
      it "fails with no closing paren" $
        parse call "" "f(1" `shouldSatisfy` isLeft
      it "fails with empty input" $
        parse call "" "" `shouldSatisfy` isLeft
